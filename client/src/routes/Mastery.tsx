...
export default function Mastery() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12">
      ...
      <p className="text-[var(--color-text-faint)] max-w-3xl mb-8">
        Per-topic EWMA mastery score (α=0.25). Updates after every check question, quiz answer, and DSA solve.
      </p>

      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <Card>
          <div className="mono text-xs uppercase tracking-widest text-[var(--color-text-faint)] mb-3">Radar</div>
          {radarPoints.length >= 3 ? (
            <Radar points={radarPoints} size={420} />
          ) : (
            <div className="text-[var(--color-text-faint)] text-sm py-12 text-center">Need more data — solve some problems first.</div>
          )}
        </Card>
        <Card>
          <div className="mono text-xs uppercase tracking-widest text-[var(--color-text-faint)] mb-3">Per topic</div>
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
            {sorted.map((t) => (
              <div key={t}>
                <div className="flex items-center justify-between text-xs mb-1">
                  <div className="font-semibold text-[var(--color-text-dim)]">{t}</div>
                  <div className="mono text-[var(--color-text-faint)]">
