...
export default function CourseDetail() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-12">
      <Link to="/courses" className="text-xs text-[var(--color-text-faint)] hover:text-[var(--color-text)] inline-flex items-center gap-1 mb-3">
      ...
      <p className="text-[var(--color-text-dim)] max-w-3xl mb-6">{course.description}</p>

      <ProgressBar value={totalLessons ? completed / totalLessons : 0} label={`Course progress · ${completed}/${totalLessons}`} />

      {course.prerequisites?.length > 0 && (
        <Card className="mt-6">
          <div className="mono text-xs uppercase tracking-widest text-[var(--color-text-faint)] mb-2">Prerequisites</div>
            ...
            <Card key={ch.id}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  ...
                  <p className="text-sm text-[var(--color-text-faint)] mt-1">{ch.summary}</p>
                </div>
                <div className="text-right">
                  <div className="mono text-xs text-[var(--color-text-faint)]">{chCompleted}/{ch.lessons.length}</div>
                </div>
              </div>
                  ...
                  return (
                    <li key={l.id}>
                      <Link
                        ...
                        className="flex items-center justify-between gap-3 p-3 rounded-lg border border-[var(--color-line)] hover:border-[var(--color-neon)]/40 transition-colors"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          {done ? (
                            <CheckCircle2 className="w-4 h-4 text-[var(--color-neon)] shrink-0" />
                          ) : (
                            <Circle className="w-4 h-4 text-[var(--color-text-faint)] shrink-0" />
                          )}
                          <div className="min-w-0">
                            <div className="text-sm font-semibold truncate">{l.title}</div>
                            <div className="mono text-xs text-[var(--color-text-faint)]">
                              difficulty {l.difficulty}/5 · {l.estimated_minutes}min · {l.check_questions.length} checks
                            </div>
                          </div>
                        </div>
                        <div className="text-xs text-[var(--color-text-faint)] shrink-0">{done ? "Done" : "Start →"}</div>
                      </Link>
                    </li>
                  );
                })}
